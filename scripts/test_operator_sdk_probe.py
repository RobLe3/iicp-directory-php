...
class ProbeTests(unittest.TestCase):
    ...
    def test_overlay_and_hook_are_test_only(self):
        ...
        self.assertEqual(overlay.count('entrypoint: [/usr/local/bin/iicp-operator-rehearsal-entrypoint]'), 2)
        self.assertEqual(overlay.count('source: ${IICP_OPERATOR_REHEARSAL_ENTRYPOINT:?set exact rehearsal entrypoint path}'), 2)
        self.assertEqual(overlay.count('target: /usr/local/bin/iicp-operator-rehearsal-entrypoint'), 2)
        self.assertEqual(overlay.count('read_only: true'), 3)
        self.assertIn('command: [php-fpm, -F]', overlay)
        self.assertIn('command: [php, artisan, schedule:work]', overlay)
        self.assertNotIn('entrypoint: !reset []', overlay)
        self.assertEqual(overlay.count('IICP_OPERATOR_REHEARSAL: "1"'), 2)
        rehearsal = (root/'operator/rehearsal-entrypoint.sh').read_text()
        self.assertIn('[ "${IICP_OPERATOR_REHEARSAL:-}" = 1 ]', rehearsal)
        self.assertIn('[ "${APP_ENV:-}" = testing ]', rehearsal)
        self.assertIn('[ "${APP_URL:-}" = http://127.0.0.1 ]', rehearsal)
        self.assertIn('load_secret APP_KEY', rehearsal)
        self.assertIn('load_secret DB_PASSWORD', rehearsal)
        self.assertIn('IICP_OPERATOR_REHEARSAL_ENTRYPOINT="$TMP/rehearsal-entrypoint.sh"', script)
        self.assertIn('install -m 0555 "$ROOT/operator/rehearsal-entrypoint.sh" "$IICP_OPERATOR_REHEARSAL_ENTRYPOINT"', script)
        self.assertIn('export IICP_OPERATOR_REHEARSAL_ENTRYPOINT', script)
