#!/usr/bin/env python3
"""Run one exact, content-free pre-1.0 qualification case for directory-php."""
...
if len(_assertions) != len(set(_assertions)):
    raise RuntimeError("qualification cases must bind unique exact assertions")
...
def command_environment(runtime_row: dict, runtime: str) -> dict[str, str]:
    ...
    release_manifest = next(
        (
            row
            for row in component["artifacts"]
            if row["kind"] == "release-manifest"
        ),
        None,
    )
